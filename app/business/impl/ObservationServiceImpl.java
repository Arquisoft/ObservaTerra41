...
public class ObservationServiceImpl implements ObservationService {
	...
	public List<Observation> findByIndicatorName(String indicatorCode) {
		Indicator ind = ServicesFactory.getIndicatorService().findByCode(
				indicatorCode);
	...
	public Observation findByCountryIndicator(String countryCode,
		...
		Indicator ind = ServicesFactory.getIndicatorService().findByCode(
				indicatorCode);
		Observation result = find.where()
				.and(Expr.eq("country", c), Expr.eq("indicator", ind))
				.findUnique();
		return result;
	}
	...
	public void addObservation(Observation ob) {
		Observation observation = ServicesFactory.getObservationService()
				.findByCountryIndicator(ob.getCountry().getCode(),
						ob.getIndicator().getCode());
		if (observation == null) {
			ob.save();
		}
	}
